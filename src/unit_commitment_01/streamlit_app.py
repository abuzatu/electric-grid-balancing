...
def create_custom_time_periods(demand_values, reserve_values):
    ...
    for t in range(len(demand_values)):
        time_periods.append({"demand": demand_values[t], "reserve": reserve_values[t]})
    return time_periods


def main():
    st.set_page_config(
        page_title="Unit Commitment Optimizer", page_icon="⚡", layout="wide"
    )
    ...
    def check_constraint_feasibility(
        ...
        demand_values,
        reserve_values,
        enable_power_balance,
        enable_reserve_capacity,
        enable_generation_limits,
        enable_ramp_up,
        enable_ramp_down,
        enable_min_up_time,
        enable_min_down_time,
    ):
        """
        ...
        if enable_power_balance:
            ...
            if total_max_power < max_demand:
                issues.append(
                    f"❌ **Power Balance Constraint:** Total maximum power ({total_max_power} MW) < Maximum demand ({max_demand} MW)"
                )
        ...
        if enable_reserve_capacity:
            ...
            max_demand_plus_reserve = max(
                demand_values[i] + reserve_values[i]
                for i in range(1, len(demand_values))
            )
            if total_max_power < max_demand_plus_reserve:
                issues.append(
                    f"❌ **Reserve Capacity Constraint:** Total maximum power ({total_max_power} MW) < Maximum demand+reserve ({max_demand_plus_reserve} MW)"
                )
        ...
        if enable_generation_limits:
            for i, unit in enumerate(units_list):
                if unit["P_min"] > unit["P_max"]:
                    issues.append(
                        f"❌ **Generation Limits Constraint:** Unit {unit['name']} - P_min ({unit['P_min']} MW) > P_max ({unit['P_max']} MW)"
                    )
        ...
        if enable_ramp_up or enable_ramp_down:
            for i, unit in enumerate(units_list):
                if unit["RU"] > unit["P_max"]:
                    issues.append(
                        f"❌ **Ramp Up Constraint:** Unit {unit['name']} - Ramp Up ({unit['RU']} MW/h) > P_max ({unit['P_max']} MW)"
                    )
                if unit["RD"] > unit["P_max"]:
                    issues.append(
                        f"❌ **Ramp Down Constraint:** Unit {unit['name']} - Ramp Down ({unit['RD']} MW/h) > P_max ({unit['P_max']} MW)"
                    )
        ...
        if enable_min_up_time or enable_min_down_time:
            for i, unit in enumerate(units_list):
                if unit["TU"] > len(demand_values) - 1:
                    issues.append(
                        f"❌ **Min Up Time Constraint:** Unit {unit['name']} - Min up time ({unit['TU']} h) > Planning horizon ({len(demand_values)-1} h)"
                    )
                if unit["TD"] > len(demand_values) - 1:
                    issues.append(
                        f"❌ **Min Down Time Constraint:** Unit {unit['name']} - Min down time ({unit['TD']} h) > Planning horizon ({len(demand_values)-1} h)"
                    )
        ...
        for i, unit in enumerate(units_list):
            ...
            if unit["u0"] and unit["p0"] < unit["P_min"]:
                issues.append(
                    f"❌ **Initial State Constraint:** Unit {unit['name']} - ON at t=0 but p0 ({unit['p0']} MW) < P_min ({unit['P_min']} MW)"
                )
            if not unit["u0"] and unit["p0"] > 0:
                issues.append(
                    f"❌ **Initial State Constraint:** Unit {unit['name']} - OFF at t=0 but p0 ({unit['p0']} MW) > 0"
                )
            if unit["su0"] and unit["u0"]:
                issues.append(
                    f"❌ **Initial State Constraint:** Unit {unit['name']} - Cannot be both ON (u0=True) and starting up (su0=True) at t=0"
                )
            if unit["sd0"] and not unit["u0"]:
                issues.append(
                    f"❌ **Initial State Constraint:** Unit {unit['name']} - Cannot be shutting down (sd0=True) when OFF (u0=False) at t=0"
                )
        ...
        for i, unit in enumerate(units_list):
            if unit["SU"] > unit["P_max"]:
                issues.append(
                    f"❌ **Startup Ramp Constraint:** Unit {unit['name']} - Startup ramp ({unit['SU']} MW) > P_max ({unit['P_max']} MW)"
                )
            if unit["SD"] > unit["P_max"]:
                issues.append(
                    f"❌ **Shutdown Ramp Constraint:** Unit {unit['name']} - Shutdown ramp ({unit['SD']} MW) > P_max ({unit['P_max']} MW)"
                )
            ...
            for i, unit in enumerate(units_list):
                ...
                if unit["P_max"] > unit["P_min"]:
                    ...
                    if ramp_time_needed > len(demand_values) - 1:
                        issues.append(
                            f"❌ **Ramp Feasibility:** Unit {unit['name']} - Needs {ramp_time_needed:.1f}h to ramp from P_min to P_max, but planning horizon is only {len(demand_values)-1}h"
                        )
        ...
        for i, unit in enumerate(units_list):
            if unit["u0"]:  # If unit is ON at t=0
                ...
                if unit["p0"] < unit["P_max"]:
                    ...
                    if ramp_time_needed > len(demand_values) - 1:
                        issues.append(
                            f"❌ **Initial State Ramp:** Unit {unit['name']} - Needs {ramp_time_needed:.1f}h to ramp from p0 ({unit['p0']} MW) to P_max ({unit['P_max']} MW)"
                        )
        ...
        if enable_min_up_time or enable_min_down_time:
            for i, unit in enumerate(units_list):
                if unit["TU"] + unit["TD"] > len(demand_values) - 1:
                    issues.append(
                        f"❌ **Min Up/Down Time:** Unit {unit['name']} - Combined min up ({unit['TU']}h) + min down ({unit['TD']}h) time > planning horizon ({len(demand_values)-1}h)"
                    )
        ...
        if enable_reserve_capacity:
            ...
            if reserve_margin < 0:
                issues.append(
                    f"❌ **Reserve Margin:** Insufficient capacity for demand + reserve. Need {max_demand + max_reserve} MW, have {total_max_power} MW"
                )
            elif reserve_margin < 10:  # Warning if very tight
                issues.append(
                    f"⚠️ **Tight Reserve Margin:** Only {reserve_margin:.1f} MW margin between max capacity and demand+reserve"
                )
    ...
    def analyze_unit_usage(units, demand_values, reserve_values, solution):
        ...
        units_by_cost = sorted(
            units_list, key=lambda x: x["c"]
        )  # Sort by variable cost
        cheapest_unit = units_by_cost[0]
        most_expensive_unit = units_by_cost[-1]

        insights.append(f"💰 **Cost Analysis:**")
        insights.append(
            f"   - Cheapest unit: {cheapest_unit['name']} (${cheapest_unit['c']}/MWh)"
        )
        insights.append(
            f"   - Most expensive unit: {most_expensive_unit['name']} (${most_expensive_unit['c']}/MWh)"
        )
        ...
        if solution and "unit_power" in solution:
            ...
            for t in range(1, len(demand_values)):  # Skip t=0
                if (
                    cheapest_unit["name"] in solution["unit_power"]
                    and t in solution["unit_power"][cheapest_unit["name"]]
                ):
            ...
            if cheapest_used:
                insights.append(
                    f"   ✅ {cheapest_unit['name']} IS being used (total: {total_cheapest_power:.1f} MWh)"
                )
            else:
                insights.append(
                    f"   ❌ {cheapest_unit['name']} is NOT being used despite being cheapest!"
                )

                # 3. Analyze why cheapest unit might not be used
                insights.append(
                    f"🔍 **Why {cheapest_unit['name']} might not be used:**"
                )

                # Check capacity constraints
                if cheapest_unit["P_max"] < max(demand_values[1:]):
                    insights.append(
                        f"   - P_max ({cheapest_unit['P_max']} MW) < max demand ({max(demand_values[1:])} MW)"
                    )

                # Check ramp constraints
                if (
                    cheapest_unit["RU"]
                    < cheapest_unit["P_max"] - cheapest_unit["P_min"]
                ):
                    insights.append(
                        f"   - Ramp up ({cheapest_unit['RU']} MW/h) might be too slow"
                    )

                # Check min up/down time constraints
                if cheapest_unit["TU"] > len(demand_values) - 1:
                    insights.append(
                        f"   - Min up time ({cheapest_unit['TU']} h) > planning horizon ({len(demand_values)-1} h)"
                    )

                # Check initial state constraints
                if cheapest_unit["u0"] and cheapest_unit["p0"] < cheapest_unit["P_min"]:
                    insights.append(
                        f"   - Initial state: ON but p0 ({cheapest_unit['p0']} MW) < P_min ({cheapest_unit['P_min']} MW)"
                    )
                ...
                if forced_units:
                    insights.append(
                        f"   - Other units forced ON: {', '.join(forced_units)}"
                    )
    ...
    if "load_example" in st.session_state and st.session_state.load_example:
        ...
        st.session_state.demand_values = [
            configs.time_periods_example[t]["demand"] for t in range(num_periods)
        ]
        st.session_state.reserve_values = [
            configs.time_periods_example[t]["reserve"] for t in range(num_periods)
        ]
        demand_values = st.session_state.demand_values
        reserve_values = st.session_state.reserve_values
        st.sidebar.success(
            "✅ Loaded Example Problem (6-period planning horizon: t=1 to t=6)!"
        )
        ...
        st.sidebar.write(
            f"🔍 Debug: Pattern loaded - num_periods={num_periods}, planning_horizon={num_periods-1}"
        )
        st.sidebar.write(
            f"🔍 Debug: Loaded {len(demand_values)} periods, first demand: {demand_values[0]}"
        )
        st.sidebar.write(f"🔍 Debug: All demand values: {demand_values}")
        st.sidebar.success(
            f"✅ Loaded {pattern_type.capitalize()} {num_periods-1}-period planning horizon (t=1 to t={num_periods-1})!"
        )
    ...
    st.sidebar.write(
        f"🔍 Debug: Slider set planning_horizon={planning_horizon}, num_periods={num_periods}"
    )
    ...
    enable_power_balance = st.sidebar.checkbox(
        "Power Balance", value=True, help="Ensure power production equals demand"
    )
    enable_reserve_capacity = st.sidebar.checkbox(
        "Reserve Capacity", value=True, help="Ensure sufficient reserve capacity"
    )
    enable_logical_consistency = st.sidebar.checkbox(
        "Logical Consistency",
        value=True,
        help="Enforce logical consistency between unit states",
    )
    enable_generation_limits = st.sidebar.checkbox(
        "Generation Limits",
        value=True,
        help="Enforce minimum and maximum generation limits",
    )
    enable_ramp_up = st.sidebar.checkbox(
        "Ramp Up", value=True, help="Enforce ramp up constraints"
    )
    enable_ramp_down = st.sidebar.checkbox(
        "Ramp Down", value=True, help="Enforce ramp down constraints"
    )
    enable_min_up_time = st.sidebar.checkbox(
        "Minimum Up Time", value=True, help="Enforce minimum up time constraints"
    )
    enable_min_down_time = st.sidebar.checkbox(
        "Minimum Down Time", value=True, help="Enforce minimum down time constraints"
    )

    # Test scenarios
    st.sidebar.subheader("Test Scenarios")
    st.sidebar.write(
        "💡 **Tip:** Try disabling 'Reserve Capacity' to see a different solution!"
    )
    st.sidebar.write(
        "💡 **Tip:** Try enabling 'Min Up/Down Time' to see if it affects the solution!"
    )

    # Display current configuration
    st.sidebar.subheader("Current Settings")
    st.sidebar.write(
        f"**Planning Horizon:** {num_periods-1} periods (t=1 to t={num_periods-1})"
    )
    ...
    with col1:
        ...
        time_df = pd.DataFrame(
            [
                {"demand": demand_values[i], "reserve": reserve_values[i]}
        ...
        time_df = time_df.rename(
            columns={"demand": "Demand (MW)", "reserve": "Reserve (MW)"}
        )
    ...
    with st.form("add_new_unit"):
        ...
        with col1_form:
            st.write("**Basic Parameters:**")
            new_p_min = st.number_input(
                "P_min (MW):", min_value=0.0, max_value=1000.0, value=50.0, step=0.1
            )
            new_p_max = st.number_input(
                "P_max (MW):", min_value=0.0, max_value=1000.0, value=200.0, step=0.1
            )
            new_ramp_up = st.number_input(
                "Ramp Up (MW/h):", min_value=0.0, max_value=1000.0, value=50.0, step=0.1
            )
            new_ramp_down = st.number_input(
                "Ramp Down (MW/h):",
                min_value=0.0,
                max_value=1000.0,
                value=50.0,
            ...
            new_startup_ramp = st.number_input(
                "Startup Ramp (MW):",
                min_value=0.0,
                max_value=1000.0,
                value=50.0,
            ...
            new_shutdown_ramp = st.number_input(
                "Shutdown Ramp (MW):",
                min_value=0.0,
                max_value=1000.0,
                value=50.0,
        ...
        with col2_form:
            st.write("**Cost Parameters:**")
            new_startup_cost = st.number_input(
                "Startup Cost ($):",
                min_value=0.0,
                max_value=10000.0,
                value=1000.0,
            ...
            new_shutdown_cost = st.number_input(
                "Shutdown Cost ($):",
                min_value=0.0,
                max_value=10000.0,
                value=500.0,
            ...
            new_variable_cost = st.number_input(
                "Variable Cost ($/MWh):",
                min_value=0.0,
                max_value=1000.0,
                value=50.0,
            ...
            new_fixed_cost = st.number_input(
                "Fixed Cost ($/h):",
                min_value=0.0,
                max_value=10000.0,
                value=100.0,
        ...
        with col3_form:
            st.write("**Minimum Up/Down Times:**")
            new_min_up_time = st.number_input(
                "Min Up Time (h):", min_value=1, max_value=24, value=4
            )
            new_min_down_time = st.number_input(
                "Min Down Time (h):", min_value=1, max_value=24, value=2
            )

        with col4_form:
            st.write("**Initial State:**")
            new_u0 = st.number_input(
                "U0 (required ON time at start):", min_value=0, max_value=24, value=0
            )
            new_d0 = st.number_input(
                "D0 (required OFF time at start):", min_value=0, max_value=24, value=0
            )
            new_p0 = st.number_input(
                "p0 (power at t=0):",
                min_value=0.0,
                max_value=1000.0,
                value=0.0,
    ...
    st.markdown("---")
    if st.button(
        "🚀 Solve Optimization Problem", type="primary", use_container_width=True
    ):

        with st.spinner("Solving the optimization problem..."):
            try:
                # Create custom time periods configuration
                custom_time_periods = create_custom_time_periods(
                    demand_values, reserve_values
                )
                ...
                issues = check_constraint_feasibility(
                ...
                if issues:
                    ...
                    for issue in issues:
                        st.write(f"  • {issue}")
                    st.write(
                        "💡 **Tip:** Fix these issues before running the solver to avoid infeasibility."
                    )
                else:
                ...
                st.write(
                    f"📊 **Problem Size:** {num_periods} periods, {len(st.session_state.units)} units"
                )
                ...
                if status == pywraplp.Solver.OPTIMAL:
                    ...
                    st.write(
                        "🔍 **Debug: Unit configuration that caused the failure:**"
                    )
                    units_debug_data = []
                    for unit_name, unit_config in st.session_state.units.items():
                        units_debug_data.append(
                    ...
                    total_max_power = sum(
                        unit_config["P_max"]
                        for unit_config in st.session_state.units.values()
                    )
                    max_demand = max(demand_values[1:])  # Skip t=0
                    ...
                    if total_max_power < max_demand:
                        st.write(
                            f"  ⚠️ **Problem:** Total maximum power ({total_max_power} MW) < Maximum demand ({max_demand} MW)"
                        )
                    ...
                    for unit_name, unit_config in st.session_state.units.items():
                        if unit_config["RU"] > unit_config["P_max"]:
                            st.write(
                                f"  ⚠️ **Problem:** Unit {unit_name} - Ramp Up ({unit_config['RU']} MW/h) > P_max ({unit_config['P_max']} MW)"
                            )
                        if unit_config["RD"] > unit_config["P_max"]:
                            st.write(
                                f"  ⚠️ **Problem:** Unit {unit_name} - Ramp Down ({unit_config['RD']} MW/h) > P_max ({unit_config['P_max']} MW)"
                            )
                    ...
                    if hasattr(solver.solver, "NumConstraints"):
                        st.write(
                            f"  • Number of constraints: {solver.solver.NumConstraints()}"
                        )
                    if hasattr(solver.solver, "NumVariables"):
                        st.write(
                            f"  • Number of variables: {solver.solver.NumVariables()}"
                        )
                    ...
                    for unit_name, unit_config in st.session_state.units.items():
                        if unit_config["P_max"] > 10000:
                            st.write(
                                f"  ⚠️ **Large P_max:** Unit {unit_name} has P_max = {unit_config['P_max']} MW (might cause numerical issues)"
                            )

                    # Check for potential logical inconsistencies
                    for unit_name, unit_config in st.session_state.units.items():
                        if (
                            unit_config["P_min"] == unit_config["P_max"]
                            and unit_config["P_min"] > 0
                        ):
                            st.write(
                                f"  ⚠️ **Fixed Output Unit:** Unit {unit_name} has P_min = P_max = {unit_config['P_min']} MW (no flexibility)"
                            )

                        if unit_config["RU"] == 0 and unit_config["RD"] == 0:
                            st.write(
                                f"  ⚠️ **No Ramp Flexibility:** Unit {unit_name} has RU = RD = 0 (cannot change output)"
                            )

                    # Check if the problem might be too complex
                    total_vars_estimate = (
                        len(st.session_state.units) * num_periods * 4
                    )  # u, p, su, sd variables per unit per period
                    if total_vars_estimate > 10000:
                        st.write(
                            f"  ⚠️ **Large Problem:** Estimated {total_vars_estimate} variables (might be computationally challenging)"
                        )
                ...
                fixed_costs = sum(
                    solver.configs_units[j]["cU"] * solver.u[j, t].solution_value()
                    for j in range(solver.num_units)
                    for t in range(1, solver.num_periods)
                )
                variable_costs = sum(
                    solver.configs_units[j]["c"] * solver.p[j, t].solution_value()
                    for j in range(solver.num_units)
                    for t in range(1, solver.num_periods)
                )
                startup_costs = sum(
                    solver.configs_units[j]["SU_cost"]
                    * solver.su[j, t].solution_value()
                    for j in range(solver.num_units)
                    for t in range(1, solver.num_periods)
                )
                shutdown_costs = sum(
                    solver.configs_units[j]["SD_cost"]
                    * solver.sd[j, t].solution_value()
                    for j in range(solver.num_units)
                    for t in range(1, solver.num_periods)
                )
                ...
                st.write(
                    f"🔍 **Debug Info:** Cost={total_cost:.2f}, Power={total_power:.1f}, Units={total_units:.0f}, Time={solver_time:.2f}s"
                )
                st.write(
                    f"💰 **Cost Breakdown:** Fixed=${fixed_costs:.2f}, Variable=${variable_costs:.2f}, Startup=${startup_costs:.2f}, Shutdown=${shutdown_costs:.2f}"
                )

                # Test constraint configuration
                st.write(
                    f"🔧 **Active Constraints:** Power Balance={solver.enable_power_balance}, Reserve={solver.enable_reserve_capacity}, Logic={solver.enable_logical_consistency}, Limits={solver.enable_generation_limits}, Ramp Up={solver.enable_ramp_up}, Ramp Down={solver.enable_ramp_down}, Min Up={solver.enable_min_up_time}, Min Down={solver.enable_min_down_time}"
                )

                # Show constraint changes
                st.write(
                    f"🔧 **Constraint Changes:** Power Balance={enable_power_balance}, Reserve={enable_reserve_capacity}, Logic={enable_logical_consistency}, Limits={enable_generation_limits}, Ramp Up={enable_ramp_up}, Ramp Down={enable_ramp_down}, Min Up={enable_min_up_time}, Min Down={enable_min_down_time}"
                )
                ...
                for t in range(1, min(4, solver.num_periods)):  # Show first 3 periods
                    total_p = sum(
                        solver.p[j, t].solution_value() for j in range(solver.num_units)
                    )
                    total_units = sum(
                        solver.u[j, t].solution_value() for j in range(solver.num_units)
                    )
                    st.write(
                        f"  t={t}: Total Power={total_p:.1f} MW, Units ON={total_units:.0f}"
                    )
                ...
                if not enable_reserve_capacity:
                    st.write(
                        "  ⚠️ **Reserve Capacity disabled** - This should allow more flexible unit scheduling!"
                    )
                if enable_min_up_time or enable_min_down_time:
                    st.write(
                        "  ⚠️ **Min Up/Down Time enabled** - This should force units to stay on/off longer!"
                    )
                if not enable_ramp_up and not enable_ramp_down:
                    st.write(
                        "  ⚠️ **Ramp constraints disabled** - This should allow faster power changes!"
                    )
                if not enable_generation_limits:
                    st.write(
                        "  ⚠️ **Generation limits disabled** - This should allow more extreme power outputs!"
                    )
                ...
                for j in range(solver.num_units):
                    ...
                    for t in range(solver.num_periods):
                        solution_dict["unit_power"][unit_name][t] = solver.p[
                            j, t
                        ].solution_value()
                        solution_dict["unit_status"][unit_name][t] = solver.u[
                            j, t
                        ].solution_value()
                        solution_dict["startup"][unit_name][t] = solver.su[
                            j, t
                        ].solution_value()
                        solution_dict["shutdown"][unit_name][t] = solver.sd[
                            j, t
                        ].solution_value()

                # Run unit usage analysis
                usage_insights = analyze_unit_usage(
                    st.session_state.units, demand_values, reserve_values, solution_dict
                )
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    with st.spinner("Generating visualizations..."):
                        # Power balance plot
                        fig1 = create_power_balance_plot(
                            solver, str(temp_path / "power_balance.png")
                        )
                        plot_files["Power Balance"] = temp_path / "power_balance.png"

                        # Unit status plot
                        fig2 = create_unit_status_plot(
                            solver, str(temp_path / "unit_status.png")
                        )
                        plot_files["Unit Status"] = temp_path / "unit_status.png"

                        # Enhanced power balance plot
                        fig3 = create_enhanced_power_balance_plot(
                            solver, str(temp_path / "enhanced_power_balance.png")
                        )
                        plot_files["Enhanced Power Balance"] = (
                            temp_path / "enhanced_power_balance.png"
                        )

                        # Total power plot
                        fig4 = create_total_power_plot(
                            solver, str(temp_path / "total_power.png")
                        )
                        plot_files["Total Power"] = temp_path / "total_power.png"

                        # Analysis plot
                        fig5 = create_detailed_analysis_plot(
                            solver, str(temp_path / "analysis.png")
                        )
                    ...
                    with tab1:
                        st.image(
                            str(plot_files["Unit Status"]), use_container_width=True
                        )
                        st.download_button(
                    ...
                    with tab2:
                        st.image(
                            str(plot_files["Detailed Analysis"]),
                    ...
                    with tab3:
                        st.image(
                            str(plot_files["Power Balance"]), use_container_width=True
                        )
                        st.download_button(
                    ...
                    with tab4:
                        st.image(
                            str(plot_files["Enhanced Power Balance"]),
                            use_container_width=True,
                        )
                        st.download_button(
                            label="Download Enhanced Power Balance Plot",
                            data=open(
                                plot_files["Enhanced Power Balance"], "rb"
                            ).read(),
                    ...
                    with tab5:
                        st.image(
                            str(plot_files["Total Power"]), use_container_width=True
                        )
                        st.download_button(
